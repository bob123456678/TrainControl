...
> **The graph window this section describes was removed in v3.0.0.** Its colours and shapes are kept here for anybody reading an older `autonomy.json`; on the track diagram a running train's route is drawn along the track instead (see [Automation.md](Automation.md)).

When the "Validate JSON" button was pressed, if the layout was valid, a visual representation was also shown, updated in real time as the paths executed.
...
* Square - trains ALWAYS turn here on arrival - a terminus station (large) or a point where reversing is compulsory (small).  Full autonomy only sends reversible locomotives here; you can still send a non-reversible one yourself if the track lets it back in.
...
Since v3.0.0 most of what is described below is set in the autonomy editor on the track diagram - a station's settings by right-clicking its square, the rest in the autonomy settings; the JSON keys are what an older `autonomy.json` carries. 
...
by right-clicking a station's square in the autonomy editor.
...
> **Superseded in v3.0.0.** Station names are now placed with **Show a Station Name Here...** in the autonomy editor; a `Point:` label that names a station the setup knows is taken over as a caption whenever the setup opens - one from an older version the first time, one typed today in the track editor the next time the setup is rebuilt - and any other stays plain text.
...
The `Return Home` button in the autonomy tab sends every locomotive back to the station it belongs at - by default the one it started on.  The same command is available by right-clicking a station on the track diagram.

Unless you say otherwise, a locomotive's home is simply the station it occupied when the layout was loaded.  Locomotives placed afterwards claim the station they are placed on, provided no other locomotive has claimed it already - each station can be the home of only one locomotive, since two locomotives wanting the same station could never both get there.  A locomotive without a home is never asked to move.  In the locomotive list, `*` marks a locomotive standing at its timetable starting point.  A locomotive standing on a station it has been *assigned* to - see below - shows its location highlighted in teal; a home that was merely derived from where a locomotive started is not marked, since that would be every locomotive on an untouched layout.
