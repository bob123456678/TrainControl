...
# The machine-readable mirror, beside the database it comes from rather than in the folder that is
# being deleted. `testEveryCitationResolves` resolves against THIS, so the guard survives the deletion
# of every document it indexes.
TSV = os.path.join("docs", "manual-tests", "findings.tsv")
...
def write(rows, orphans=None, used=None, outside=None, elsewhere=None):
    ...
    write_the_mirror(rows, orphans)


def write_the_mirror(rows, orphans=None):
    """The refs as plain text, for the guard and for grep.

    `testEveryCitationResolves` resolves every citation in the codebase against this file. It cannot
    read the database - there is no SQLite driver on this project's classpath - and the markdown
    catalogue lives in the folder being deleted, so this is what makes the promise enforceable after
    the reviews are gone.

    The disposition is written as the document stated it, which is NOT its status today: nothing has
    ever updated one. See the header the file carries.

    :param rows: the catalogue
    :param orphans: refs that are cited and are findings nowhere
    """
    # AND NOT OVER THE RENDER THE STORE MAKES (VD16-C4).
    #
    # There are two writers of this file.  This one writes three columns from a catalogue read out of
    # the review documents; `triagedb.render_findings` writes five from the `finding` table, and that
    # is what the `--add` path and every other caller uses now.  This path only runs on the full
    # rebuild, which already refuses unless `--force` - but if it ever does run over a rendered
    # mirror it silently drops the column that says what each finding was about, on the only file
    # Java can read the catalogue from.
    if os.path.exists(TSV):
        for line in io.open(TSV, encoding="utf-8"):
            if line.startswith("#") or not line.strip():
                continue

            if len(line.split(chr(9))) > 3:
                raise SystemExit(
                    "REFUSING to write a three-column mirror over the one the store renders."
                    " docs/manual-tests/findings.tsv comes from the finding table now:"
                    " python -c " + chr(34) + "import triagedb;"
                    " triagedb.render_findings(triagedb.connect())" + chr(34))

            break
    out = ["# Every review finding. Generated by docs/tools/catalog-findings.py - do not hand-edit.",
           "#",
           "# ref<TAB>document<TAB>disposition-as-that-document-stated-it",
           "#",
           "# The disposition is what the document said ON THE DAY IT WAS WRITTEN. It is not status.",
           "# 63 A-severity findings read `open` on 2026-09-08. Thirteen were checked against the",
           "# code that day and all thirteen had been fixed; of the rest, 32 are named by a fixing",
           "# commit or adjudicated in a later document. Read this file to find out WHAT a citation",
           "# referred to, never whether it is still true - for that, read the code.",
           "#",
           "# Refs below the DEAD marker are cited from the code and are findings in no document."]

    for r in sorted(rows, key=lambda r: (r["ref"], r["document"])):
        out.append("%s\t%s\t%s" % (r["unique"], r["document"],
                                    " ".join((r["disposition"] or "-").split())[:90]))

    out.append("# DEAD - cited, no finding behind them")

    for ref in (orphans or []):
        out.append("%s\t-\t-" % ref)
...
# Below this many documents, the reviews folder is gone rather than shrunk. It held 120.
ENOUGH_DOCUMENTS = 20
...
def main():
    ...
    # This script scans `docs/reviews/`; with the documents gone it collects nothing, and every step
    # after this point WRITES - the markdown, then the mirror the Java guard reads, then the database.
    # The mirror and the database are the only two copies left. Stop before the first write, not after.
    seen = len(set(r["document"] for r in rows))

    if seen < ENOUGH_DOCUMENTS and "--force" not in sys.argv:
        print("Found %d findings in %d documents under %s." % (len(rows), seen, REVIEWS))
        print()
        print("REFUSING TO RUN. The review documents were deleted on 2026-09-08 and this scans them,")
        print("so it now produces an empty catalogue - and everything below this line overwrites:")
        print("  %s" % OUT)
        print("  %s   <- what testEveryCitationResolves resolves against" % TSV)
        print("  the finding table in docs/manual-tests/triage.db")
        print()
        print("Those last two are the only record of what the reviews found. To regenerate the mirror")
        print("from the database, which is what you almost certainly want:")
        print()
        print('  python -c "import sys; sys.path.insert(0, \'docs/manual-tests\'); import triagedb;'
              ' triagedb.render_findings(triagedb.connect())"')
        print()
        print("To rebuild the catalogue, restore docs/reviews/ from git history first. Pass --force")
        print("only if you have a reason to write an empty catalogue over a full one.")
