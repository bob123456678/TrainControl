...
The findings themselves, as reported. What was done about each is in
`2026-08-21-review-dispositions.md`; this file is the register.

Two reviews ran against `1efa3b9a`, both read-only, neither compiling or running anything:
...
Every finding below was re-verified against the source by me before it was acted on. Findings that
did not survive that verification are listed at the end rather than dropped.

**Severity is mine, not the reviewers'** — both ranked their findings but neither labelled them. The
scale:

| | |
|---|---|
| **Critical** | Destroys the user's work, or commands the railway wrongly, without saying so |
| **High** | Wrong behaviour in ordinary use, or a fault that persists for the rest of the session |
| **Medium** | Wrong or unusable in one flow, recoverable once noticed |
| **Low** | Cosmetic, unreachable today, or a documented trade-off |
...
| M6 | **A locked route's explanation is overwritten immediately after construction.** `becomeReadOnly()` sets a title saying why every control is grey; `editRoute` then sets the ordinary edit-route title over it. The old editor's Save-button tooltip that used to carry the same sentence is gone too. | `TrainControlUI.java:13924`; `RouteEditorFrame.java:1626` | R13 | CONFIRMED | Fixed `174178c5` |
| M7 | **"Show Station Here" tooltip says the action is refused, on an item that performs it.** `setEnabled(mine)` was removed in favour of a confirm dialog; the tooltip telling the user to go and clear the text elsewhere stayed. It sends them into the dead end the change removed. | `AutonomyEditorPanel.java:1415`; `messages.properties:1643` | R16 | CONFIRMED | Fixed `174178c5` |
| M8 | **The save-immediately mechanism reached the move paths and not the forget paths beside them.** The three `forgetCaptionsAt` call sites (paste, fill, delete) leave the change in memory only; it reaches disk solely through `resetAutonomySession`, which is gated on a configuration being loaded and the layout not running. | `LayoutEditor.java:2070`, `:2359`, `:2497` | R17 | PLAUSIBLE | Fixed `174178c5` |
| M9 | **`updatePowerState` touches Swing straight from the message thread** — the one `View` callback in that class that does not marshal. Intermittently stuck Go/Stop button states. | `TrainControlUI.java:4927` | P (minor) | CONFIRMED | Fixed `174178c5` |
| M10 | **`AutoLocomotiveStatus` can NPE on the EDT** reading the last milestone for a locomotive whose driver has already cleared them (the two maps are cleared in sequence, unsynchronized). The exception lands inside the unguarded loop that refreshes every panel, so every panel after it is left stale. | `AutoLocomotiveStatus.java:275`; `TrainControlUI.java:18450` | P (minor) | CONFIRMED | Fixed `174178c5` |
| M11 | **A popup diagram window never discards the grid it replaces**, so the outgoing grid's failsafe (8s) and grace (120ms) timers go on mutating the panel the replacement now owns — the glitch `discard()` was added to remove, on the one path that did not call it. | `LayoutPopupUI.java:41` | P (minor) | CONFIRMED | Fixed `174178c5` |
| M12 | **`NetworkProxy.model` is assigned after the control station's constructor has already transmitted.** A send failure in that window NPEs inside the catch block, out of the constructor and out of `main`, which prints `Error occurred: null` and exits. | `NetworkProxy.java:119`; `MarklinControlStation.java:335`, `:3594` | P1 (related) | CONFIRMED (window), PLAUSIBLE (trigger) | Fixed `174178c5` |
| M13 | **Executors, socket and timers are never shut down** — three non-daemon single-thread pools, a `DatagramSocket` nothing closes, a non-daemon reader thread whose only exit is that socket closing, and a discarded `Timer`. The GUI masks it with `System.exit(0)`; the documented programmatic entry point hangs on return, and a second `init()` in one JVM leaks the port. | `MarklinControlStation.java:243`; `TrainControlUI.java:4158` | P (minor) | CONFIRMED | **Deferred** |

## Low

| # | Finding | Where | From | Conf. | Status |
|---|---|---|---|---|---|
| L1 | **Two ordinary adjacent turnouts with a settle delay open as one three-way row.** `Switch 5 straight,300` then `Switch 6 turn` — a common crossover — matches the RIGHT shape exactly. The round trip is byte-exact so nothing is lost, but deleting that row removes both commands and changing it to "left" writes a different piece of railway. | `RouteEditorFrame.java:836`; `ThreeWaySwitch.read` | R (minor) | CONFIRMED | **Declined** — documented trade-off of reading points back out of commands |
| L2 | **The digits-only address editor covers Accessory and Feedback but not Signal or Three-way**, so those accept "twelve" and complain only at Save. | `RouteEditorFrame.java:2627` | R (minor) | CONFIRMED | **Declined** — validation catches it |
| L3 | **`AutonomyBanner.hold()` stores the rendered placeholder into `saying`**, so `isSaying()` returns true forever after the first message. | `AutonomyBanner.java:344` | R (minor) | CONFIRMED | **Declined** — no caller anywhere in `src` or `test` |
| L4 | **`snapshotPage` puts live per-point `JSONObject` references into the undo snapshot**, and `setPointProperty` mutates them in place. Reachable only by editing an autonomy property between a diagram snapshot and its undo; the two modes are otherwise exclusive. | `AutonomyCompanionStore.java:1385` | R (minor) | CONFIRMED | **Declined** — `snapshotSetup`, added in this batch, deep-copies and is the model if it ever needs fixing |
| L5 | **The page cache branch replaces a panel's contents without discarding the outgoing grid**, so its 120ms grace timer can still drop a spinner into the panel. | `TrainControlUI.java:19391` | R7 (related) | CONFIRMED | **Declined** — same shape as M2, no observed symptom on that path |

---

## Reported and rejected on verification

Kept here because a rejected finding is worth as much as a confirmed one — it stops the same claim
being re-raised.

| Claim | Why it does not hold |
|---|---|
| `CS2File.sanitizeFilename` does not replace `/` and `\`, so a configuration name containing a path separator escapes the folder | It does. The character class covers both. Rejected by reviewer P before reporting; re-checked here literally. |
| `MarklinAccessory.parseMessage` applies state after `updateTiles`, so a throw in the tile loop loses the state | The order is the other way round: state is applied first. Rejected by reviewer P before reporting. |
| The multi-signal widening leaves callers assuming a single signal | Traced end to end by R: the store's list accessors, `rekeyListValues`/`moveListValues` (both `setValue` a fresh list, so page snapshots are not aliased), `readStringListMap`, `protectingSignalNames`, `signalsThatAreGone`, `AutonomyBuilder:801`, `Point.toJSON:786`, `Layout.refreshOneSignal`. `getProtectingSignal()` singular is now referenced only from tests. The only defect in that work is C10. |

## Explicitly checked and clean

Reported by R as searched and found sound, worth recording so it is not re-swept:

- the signal picker dialog does not nest modals; the click → add → reopen path fully unwinds before
  the next click, and all four option labels are distinct in every locale
- `RouteCapture` is a faithful lift of the deleted editor's filter; `ThreeWaySwitch` expand/read are
  exact inverses for all three positions
- `TileAnnotation`, `RowIcons`, `LayoutRightclickAutonomyMenu`, and the `TEST_CS2_ADDRESS` change
- save-path field coverage against the deleted `RouteEditor`: name, s88, trigger, enabled, commands and
  conditions all reach the same `editRoute`/`newRoute` signatures, and the route id is preserved
- all eight message bundles are pure ASCII with identical key sets, and every key referenced from code
  resolves
...
|---|---|---|---|---|---|
| Fixed in `174178c5` | 10 | 9 | 12 | 0 | 31 |
| Deferred | 1 | 3 | 1 | 0 | 5 |
| Declined | 0 | 0 | 0 | 5 | 5 |
| **Total** | **11** | **12** | **13** | **5** | **41** |

Of the 41, one (C3) was found by me while verifying the reports rather than by either reviewer.

The five deferred items are the four in `traincontrol-deferred-reliability` plus M13; each needs a
number — a timeout, a lock boundary — that is a judgement about the railway rather than about the code.
