...
| UIX-C2 | open | `LayoutEditor.java:7320-7325`, the Control+B comment, against the post-processor's own guard at `TrainControlUI.java:9395` |
| UIX-C3 | open | `GraphLocAssign.java:377`; `AutonomyBanner.java:211` |
| UIX-C4 | open | the eight `messages*.properties` bundles: about 237 keys nothing asks for |
| UIX-C5 | open | `ui.main.tooltip.switchDir` / `switchDirFwd` (`messages.properties:1533-1534`) against `LeftArrowLetterButtonPressed` / `RightArrowLetterButtonPressed` (`TrainControlUI.java:23396-23403`) |
