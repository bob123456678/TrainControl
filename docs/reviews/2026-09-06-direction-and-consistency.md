...
| **Disposition** | fixed - Adam, 2026-09-06: **"flipFacing should also update the running layout."**  `flipFacing` now takes the layout and moves the locomotive onto the copy of its square that faces the new way, in the same method as the setup write, because that is the only arrangement in which the two cannot drift.  Not through `moveLocomotive` - that refuses while anything is running, requires a destination which a split copy need not be, and adds to the run list; the train is already placed and only changes which copy it stands on. |
