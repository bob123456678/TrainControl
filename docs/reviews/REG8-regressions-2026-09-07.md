...
**Dispositions checked against HEAD on 2026-09-08.**  They were all still reading Open while the
commits that fixed them carried the finding ids - which is the complaint REG8-C6 itself made about
REG7's table, made again about this one.  A status column nobody updates is worse than none: it is
read as a backlog and it is a work of fiction.
|---|---------|----------|------------|-------------|
| REG8-A1 | Every build or reload silently erases `arrivedFrom` from every occupied square: `parseAuto` applies it and then `setLocomotive` - carrying `8f006b3a`'s own occupant-change clear - wipes it in the same loop iteration. The test that "proves" survival asserts the JSON string and the session store, never the parsed railway | A | Confirmed (trace) | Fixed 2026-09-07 (95c09ac4) - the build applies the arrival side AFTER placing the locomotives, beside the FR-001 blockers, so the placement can no longer wipe it |
| REG8-B1 | `reconcileFacingWhenIdle` (from `bc6120f1`) levels the direction baseline from live state on any idle refresh; interleaved with a genuine operator reversal it can swallow the one kind of direction command the graph must follow, and the "no backlog" rule holds only if a refresh happens to run between run-end and the next echo | B | Possible (needs a run) | Open - the late-echo race, still only reachable by a run.  Same finding as IND9-B6 |
| REG8-B2 | The diagram's Edit/Assign-locomotive door records the placement in the running layout only - no `session.placeLocomotive`, no `setFacing` - while its editor twin was given both this week. A placement made there reverts on the next configuration load unless an editor capture happens first | B | Confirmed (trace) | Fixed 2026-09-07 (51a6a971) - both doors go through GraphLocAssign.commitAndRecord, which writes the placement AND the facing to the session |
| REG8-C1 | `behaviour.md` contradicts itself twice, both left by `8ea1ff15`'s revision: §3 says a mid-run reversal is both "followed once the run ends" and "ignored, not queued"; §4 says the arrival side "can be set or cleared" six lines above "Clearing is not offered" | C | Confirmed | Fixed - behaviour.md carries one rule for each; the arrival-side half was settled by Adam's "clearing should not be possible, only setting" |
| REG8-C2 | `behaviour.md` §3 claims "a journey to a may-reverse destination is still refused if the operator declines the turn it needs"; `d45d7951` removed the entire refusal mechanism and nothing enforces it - though tracing shows the case cannot arise, so the doc describes machinery that neither exists nor is needed | C | Confirmed | Fixed - the refusal sentence is gone from behaviour.md |
| REG8-C3 | The call-site comment above the room check in `isPathClear` still states the withdrawn all-or-nothing rule ("a path carrying any unmeasured segment is not judged at all") that `4499dddc` replaced in the method it introduces | C | Confirmed | Fixed 2026-09-08 (MON-C6 sweep) - the call-site comment and three javadocs now state the total-of-what-is-measured rule that replaced it |
| REG8-C4 | `testADirectionChangeIsNotSwallowed`'s stated rationale is `ca0265f4`'s withdrawn "deferred until safe" ruling; it enforces `putIfAbsent` + guard-first, which is only correct in combination with the idle reconcile, and nothing ties the two together | C | Confirmed | Fixed 2026-09-07 (1bd81f53) - the test's rationale names the idle reconcile, and its guard asserts the ordering the two depend on together |
| REG8-C5 | Three orphan bundle keys were carefully reworded this week (`layout.ui.tooltipGrowDiagram`, `tooltipShrinkDiagram`, `layout.ui.errorNoStationsInGraph`) - no Java in `src/` or `test/` reads any of them, and none did 200 commits ago | C | Confirmed | Fixed 2026-09-07 (1bd81f53) - the three orphan keys are gone from all eight bundles |
