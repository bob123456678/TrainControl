...
Run against `cs2_sample_layout` (`testAutonomyDiagramSampleLayout`): **every hand-built connection is
...
regenerates. Any future change to the port map must update both, and `testAutonomyDiagramPorts` must agree
with both.
...
- `testAutonomyDiagramPorts` asserts internal consistency (every port group side is reciprocal-capable;
...
1. NetBeans compile; `testAutonomyDiagramPorts`, `testAutonomyDiagramTiles`, `testAutonomyDiagramReducer`,
   `testAutonomyDiagramStore`, `testAutonomyBuilder`.
...
**R1 (UI)**:
4. `testAutonomyDiagramMonitor`, plus `testLayoutTiles`, `testHomeStaging`, `testAutoLayout` for regression.
