...
| SWITCH_THREE | `threeway` | N E S W | one group; common + 3 branches, **two addresses** (`threeway_active`, `threeway_active2`) | M / **D** |
...
2. **Branch → accessory setting.** Diffing each `_active` variant against its base shows the
   ...
   (STRAIGHT/TURN, GREEN/RED) selects it. The common leg vs branch legs assignment, and the
   setting that picks each, are **declared and must be confirmed against real hardware behavior**.
...
### Port-map correctness strategy (the load-bearing risk)
