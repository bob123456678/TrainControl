...
| Put a station name on the diagram | layout editor only (`editTextWithDropdown`), text squares only | Automatic: marking a square a station, or naming one, writes the label - on the square if its rails run east-west, on the square below otherwise.  Manual placement stays on the right-click menu for anywhere else | DONE (new) |
