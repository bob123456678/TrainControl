...
* v3.0.0 [Beta]
    - Features
        - Autonomy
            ...
            - Station names you had typed onto the track diagram in an earlier version (the ones written as "Point:name") are taken over by the new setup the first time it opens, and removed from the page file so they are not shown twice.  A copy of each page as it was is kept beside it with a .bak extension, and the log says which pages were changed.  A name that no longer matches any station is left exactly where it is
