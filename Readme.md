...
* v3.0.0 [Beta]
    - Features
        - Autonomy
            ...
            - Autonomy setups are saved as named configurations that can be duplicated, renamed, deleted, exported and imported, and the one you were last using is loaded when TrainControl starts, unless Load Autonomy has been unticked.  A layout still set up the old way, with an autonomy.json, is loaded at start only where Load Autonomy was ticked by hand, as before.  An autonomy.json from an older version can be imported from the same menu; importing one unticks Load Autonomy, so tick it again to have the imported setup loaded at start.  On a layout stored on this computer, TrainControl no longer writes autonomy.json; the setup is saved in the layout folder.
